    ...
    if (!marketData) {
        return (
            <Card title={t('marketData.title')} icon={<ChartBarIcon className="w-6 h-6" />}>
                <p className="text-gray-400">{t('market.fetching')}</p>
            </Card>
        );
    }
        ...
        <Card title={t('marketData.title')} icon={<ChartBarIcon className="w-6 h-6" />}>
            <div className="space-y-5">
                <div className="text-center">
                    ...
                    <p className="text-3xl font-bold text-white">{formatPrice(marketData.priceUSD)}</p>
                </div>

                <div className="grid grid-cols-2 gap-4 text-center text-sm">
                    <div>
                        <p className="text-gray-400">{t('marketData.mcap')}</p>
                        <p className="font-semibold text-gray-200">{formatCurrency(marketData.marketCapUSD)}</p>
                    </div>
                    <div>
                        <p className="text-gray-400">{t('marketData.volume')}</p>
                        <p className="font-semibold text-gray-200">{formatCurrency(marketData.volume24hUSD)}</p>
                    </div>
                </div>
                 <div className="text-center text-xs text-gray-500">
                    <span>{t('marketData.source')}: {marketData.provider} ({t('marketData.simulated')})</span>
                </div>
            </div>
        </Card>
    );
